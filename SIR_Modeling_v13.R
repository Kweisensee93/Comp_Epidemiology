...
init2<- c(S = 1-1e-6, I= 0.0, Tr = 1e-6, Ut = 1e-6, R = 0.0)
  # Time frame
...
sir_t <- function(time, state, parameters) {
  ...
  with(as.list(c(state, parameters)), {
    ...
    dI <- (beta * S * (Ut+Tr) - beta * (1-zeta) * S * Tr) -eff * gamma * Tr - gamma * Ut
    ...
    return(list(c(dS, dI, dTr, dUt, dR)))
  })
}
...
for (i in 1:5){
  ...
  data.sir_t <- melt(out,id="time",measure=c("S", "I", "Tr","Ut","R"))
  ...
  plot_treatment[[i]] <- ggplot(data.sir_t)+
    ...
    theme(text = element_text(size = 15)) +
    ylim(0, 1) +
    scale_color_manual(values = c("#808080", "#FFAE42","#FF10F0","#006942", "#4aa5d9")) +
    theme(plot.title = element_text(size = 12, face = "bold"))
}
